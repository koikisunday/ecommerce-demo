...
async function main() {
  const admin = await prisma.user.upsert({
    ...
    update: {},
    create: { email: 'admin@example.com', name: 'Admin', password: 'changeme', role: 'ADMIN' }
  })

  const vendorUser = await prisma.user.upsert({
    where: { email: 'vendor@example.com' },
    update: {},
    create: { email: 'vendor@example.com', name: 'Vendor Owner', password: 'changeme', role: 'VENDOR' }
  })
  ...
  await prisma.order.create({
    data: {
      ...
      totalAmount: 2000,
      status: 'PENDING',
      items: { create: [{ productId: 1, quantity: 1, unitPrice: 2000 }] }
    }
  })
}
